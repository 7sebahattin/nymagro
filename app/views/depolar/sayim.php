...
<style>
  ...
  .sayim-card { background: #fff; border: 1px solid #ddd; border-radius: 4px; overflow: hidden; box-shadow: 0 2px 5px rgba(0,0,0,0.05); }
  ...
  .sayim-table th { background: #f8fafc; padding: 12px 15px; border-bottom: 2px solid #eee; text-align: left; font-size: 12px; font-weight: 600; color: #555; }
  .sayim-table td { padding: 10px 15px; border-bottom: 1px solid #f1f5f9; font-size: 13px; vertical-align: middle; }
  
  .input-sayim { width: 100px; padding: 6px 10px; border: 1px solid #ccc; border-radius: 4px; outline: none; font-weight: 700; text-align: center; color: #337ab7; }
  ...
  .info-box { background: #d9edf7; color: #31708f; border: 1px solid #bce8f1; padding: 15px; border-radius: 4px; margin-bottom: 20px; font-size: 13px; }
</style>
...
<div class="page-container">
  ...
  <form action="<?= BASE_URL ?>/depo/sayimKaydet/<?= $depo['id'] ?>" method="POST">
    <div class="sayim-card">
      ...
      <table class="sayim-table">
        ...
        <tbody>
          <?php if (empty($stoklar)): ?>
            <tr><td colspan="5" style="text-align:center; padding:30px; color:#999;">Bu depoda sayılacak ürün bulunamadı.</td></tr>
          <?php else: ?>
            <?php foreach ($stoklar as $s): ?>
              <tr>
                <td style="font-weight:600;"><?= htmlspecialchars($s['urun_adi']) ?></td>
                <td style="color:#777; font-size:12px;"><?= htmlspecialchars($s['stok_kodu'] ?: '-') ?></td>
                ...
                <td style="color:#777;"><?= htmlspecialchars($s['birim']) ?></td>
              </tr>
            <?php endforeach; ?>
          <?php endif; ?>
        </tbody>
      ...
      <div style="padding:20px; text-align:right; background:#f8fafc; border-top:1px solid #eee;">
        <a href="<?= BASE_URL ?>/depo/detay/<?= $depo['id'] ?>" style="color:#777; font-size:13px; margin-right:20px; text-decoration:none;">Vazgeç</a>
        <button type="submit" class="btn-save">
          <i class="fa-solid fa-check"></i> Sayımı Kaydet ve Stokları Güncelle
        </button>
      </div>
